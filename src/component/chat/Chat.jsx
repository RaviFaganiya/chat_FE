...
const Chat = ({
  ...
  const handleMessageSent = useCallback((message) => {
    setMessages((prev) =>
      prev.map((msg) => (msg._id === message.tempId ? { ...msg, ...message, type: "sent" } : msg))
    )
  }, [])

  const handleReadStatus = useCallback(({ messageId }) => {
    setMessages((prev) =>
      prev.map((msg) => (msg._id === messageId ? { ...msg, isRead: true } : msg))
    )
  }, [])

  const handleDeliveryStatus = useCallback(({ messageId }) => {
    setMessages((prev) =>
      prev.map((msg) => (msg._id === messageId ? { ...msg, isDelivered: true } : msg))
    )
  }, [])
  ...
  const handleVoiceModalOpen = () => setShowVoiceModal(true)
  ...
  const getMessageStatus = (msg) => {
    if (msg.type !== "sent") return null
    if (msg.isRead) return <span title="Read">✓✓</span>
    if (msg.isDelivered) return <span title="Delivered">✓✓</span>
  ...
  return (
    <div className="flex flex-col h-full w-full p-6 relative">
      ...
      <div className="mb-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <img
            src={toUser?.profilePic || "/placeholder.svg"}
            alt={toUser?.name}
            className="w-10 h-10 rounded-full object-cover"
          />
          <div>
      ...
      <div className="flex-1 overflow-y-auto mb-4 border rounded p-4 bg-white">
        {messages.map((msg) => (
          ...
          >
            <div className="flex justify-between items-center gap-2">
              <span>{msg.message}</span>
              {msg.type === "sent" && <span className="text-xs ml-2">{getMessageStatus(msg)}</span>}
            </div>
            <div className="text-xs mt-1 opacity-70">
              {new Date(msg.timestamp).toLocaleTimeString([], {
                hour: "2-digit",
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={!input.trim()}
          className={`px-4 py-2 rounded ${
            input.trim() ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-300 text-gray-500"
          }`}
        >
          Send
        </button>
