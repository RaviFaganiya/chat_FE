...
export default function Sidebar({ setIsCreateAccount, onProfileClick }) {
  ...
  useEffect(() => {
    ...
    // Socket event listeners for real-time updates
    const handleLastMessageUpdate = ({ userId, lastMessage }) => {
      console.log("Sidebar: Last message update:", { userId, lastMessage })
    ...
    const handleUnreadCountUpdate = ({ userId, unreadCount }) => {
      console.log("Sidebar: Unread count update:", { userId, unreadCount })
  ...
  const handleUserClick = async (userId) => {
    console.log(`Sidebar: User clicked: ${userId}`)
    navigate(`/chat/${userId}`)
  }
  ...
  // Enhanced time formatting for sidebar
  const formatTime = (timestamp) => {
    const now = new Date()
    const messageTime = new Date(timestamp)
    ...
    if (diffInMinutes < 1) {
      ...
      return dayNames[messageTime.getDay()]
    } else {
      return messageTime.toLocaleDateString([], { month: "short", day: "numeric" })
    }
  }
    ...
    <div className="w-64 h-screen bg-gray-900 text-white flex flex-col">
      ...
      <div className="p-4 border-b border-gray-700">
        ...
        <div className="flex items-center gap-3 p-3 bg-gray-800 rounded-lg mb-3">
          <div className="relative">
            <img
              src={data?.currentUser?.profilePic || "/placeholder.svg?height=40&width=40"}
              alt="Profile"
              className="w-10 h-10 rounded-full object-cover"
            />
          </div>
